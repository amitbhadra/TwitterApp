...
let setRoutingTableEntry (route_table: string [][]) (neighbor_idx: string) (self_idx: string) (longest_prefix: int)=
    ...
    if longest_prefix <> routing_table_size && neighbor_idx <> "" && self_idx <> "" then
        ...
        if next_entry_neighbor <= 7 then
            if route_table.[longest_prefix].[next_entry_neighbor] = "" then
                ...
                let mutable current_diff = Math.Abs(Convert.ToInt32(route_table.[longest_prefix].[next_entry_neighbor]) - Convert.ToInt32(self_idx))
...
let myPastryActor (mailbox: Actor<_>) =
    let rec loop actor_idx actor_key leaf_set routing_table neighborhood_set oldtime localtime = actor {
        ...
        | PastryInit(idx, nearest_node_idx) ->
                        ...
                        let n_bits = int(ceil(Math.Log(float(num_nodes), 8.0))) + 1
        ...
        | ReceiveRouteMessage(other_routing_table, other_set, other_actor_key, num) ->
                        ...
                        if num = 0 then
                            ...
                            for i in 0..routing_table.Length do
                                for j in 0..routing_table.[i].Length do
...
let myBossActor (mailbox: Actor<_>) =
    let rec loop() = actor {
        ...
        | SendMessageBoss(max_nodes, num_requests) ->
                        ...
                        num_nodes <- max_nodes
                        routing_table_size <- (Math.Log(float(num_nodes), 8.0) + 2.0) |>int 
                        ...
                        for i in 0..8 do
                            ...
                            let mutable sync = Async.RunSynchronously(res, 10) |> string
                            0|>ignore
                            //Thread.Sleep(100)
                        printfn "9"
